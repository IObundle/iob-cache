...
`endif
   reg 						  data_load;
   reg [OFFSET_W-1 :0] 				  select_counter, select_counter_aux;
   ...
   wire [TAG_W-1:0] 				  tag_addr_word = cache_addr[ADDR_W-1:(ADDR_W-TAG_W)];
   ...
   generate
      for (i = 0; i < 2**OFFSET_W; i=i+1)
	   ...
	   data_memory 
		...
		.mem_write_data(write_data),
		.mem_addr      (index     ),
		.mem_en        ((i == word_select)? data_line_wstrb : {N_BYTES{1'b0}}), 
   ...
   always @ (posedge clk, posedge reset)
	...
	if (reset)
	  ...
	  case (read_state)
		 ...
		 select_counter_aux <= {OFFSET_W{1'b0}};
